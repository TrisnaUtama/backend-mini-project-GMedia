...
const store = [
    check('category_id').isUUID().notEmpty().withMessage('Category is required'),
    ...
    check('price').notEmpty().withMessage('Price is required').isFloat({ gt: 0 }).withMessage('Price must be a number greater than 0'),
    check('stock').notEmpty().withMessage('Stock is required').isInt({ gt: -1 }).withMessage('Stock must be an integer 0 or greater'),
    (req, res, next) => {
